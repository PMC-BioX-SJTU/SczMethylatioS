...
def met_nor_result(all_data,trio_3_person_norm_index,outfile1,trio_3_person_fa_index,outfile2,trio_3_person_ma_index,outfile3,outfile4):
    ...
    ma_matrix = [[0 for col in range(27)]for row in range(252699)]
    ma_matrix[0][0]='111'
    ma_matrix[0][1]='110'
    ma_matrix[0][2]='11-1'
    ma_matrix[0][3]='101'
    ...
    for line in islice(all_data,2,None):
        ...
        for i in range(0,len(trio_3_person_ma_index),3):
          ...
          mode=line[trio_3_person_ma_index[i]]+line[trio_3_person_ma_index[i+1]]+line[trio_3_person_ma_index[i+2]]
          ...
          ma_matrix[n][t] += 1
        n = n+1
    np.savetxt(outfile_ma,ma_matrix,fmt='%s')
    #np.savetxt(outfile_in,inhert_matrix,fmt='%s')
    all_data.close()
    outfile_ma.close()
    return ma_matrix,all_modes
...
def figure(met_result_matrix,all_modes):
    ...
    plt.ylabel("Count")
    plt.title("Trio (Mather)")

    plt.show()
    plt.savefig("Mather_all_modes_bar.jpg",dpi=300)
    plt.savefig("Mather_all_modes_bar.pdf", bbox_inches='tight')
